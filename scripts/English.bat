...
color 0a
:: First-time execution script: download resource files
if not exist ".\Temp-dataset\configs\config.yaml" (
echo Detected first-time script execution, downloading resource files and running example tasks, please wait patiently for 1-3 min...
...
echo=
set /p tmp=Please press Enter to open the parameter file and start configuring your own task...
copy /y ".\Temp-dataset\configs\config.test.yaml" ".\Temp-dataset\configs\config.yaml" > nul
start "" ".\Temp-dataset\configs\config.yaml"
echo=
echo /p tmp=Please press Enter to enter the main menu...
cls
) else (
echo=
)
